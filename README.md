...
| Trainable params | **~27.3M** (stripper + injector), and the DAC codec stays frozen |
...
> is the most informative part, and it isn't hidden.
...
1. **Conditioned on BOTH `src_midi` and `tgt_midi`** (not the interval). Each is mapped through
   a `SinusoidalPitchEmbedding(256)`, and the two are concatenated into a 512-dim conditioning
...
2. **Exponential dilations `[1,2,4,8,16,32]`** across the 6 blocks give a receptive field of
   ~132 of the 172 frames. Pitch is a global property of the 2 s window, whereas a plain `kernel_size=3`
...
Per-frame cosine gives every frame equal directional pressure regardless of its magnitude, and
...
"same" instrument, where room/mic/string variation put a noise floor under val loss - see §8.)
...
- `--source-midi` **(required)** - the MIDI note of the source clip, which the injector needs.
...
ships CUDA builds, and reinstalling from PyPI can silently swap in a CPU-only build).
...
1. **DAC is frozen, and we only train a latent-space adapter.** No audio reconstruction loss, no
   decoder in the gradient path. L1/cosine on latents is sufficient and far cheaper.
2. **Stripper has no gradient, and the injector does all the work.** This sidesteps the adversarial
...
3. **Inject on (src_midi, tgt_midi), not the interval.** The latent pitch transform is
   register-dependent, and interval-only conditioning cannot represent it.
...
6. **Pairs grouped by (instrument, velocity), with identity pairs included.** Guarantees only pitch
...
Sound quality is the product spec here, so the first three whole paradigms below were rejected on
fidelity before modelling even mattered. The narrative version is in
[`docs/CASE_STUDY.md`](docs/CASE_STUDY.md) §2.
- **DDSP (differentiable additive synthesis):** the harmonic+noise prior flattens inharmonicity,
  hammer-attack noise, and resonance into a "synthy/organ-like" timbre, and the ddsp+CREPE pipeline
  caps at 16 kHz. Tried twice. Disqualified on timbre.
- **RAVE (realtime audio VAE):** its adversarial realtime decoder trades fidelity for speed -
  "fuzz"/grain and unstable pitch on sustained tonal notes. Also never trained (no MPS, CPU fallback
  hopeless, native run failed).
- **Mel-spectrogram + neural vocoder (HiFi-GAN/BigVGAN):** two stacked lossy steps (mel discards
  phase, and the vocoder adds artefacts on sustained musical tones, since its reputation comes from speech/TTS),
  plus brittle mel↔vocoder config matching. This is what pointed toward a single high-fidelity
  invertible representation end-to-end (a frozen DAC).
...
- **DAC decoder in the training loop (audio loss):** removed, since latent loss is enough and much
  cheaper.
...
- Listen-test after ~20 epochs, assessing pitch-transfer quality across registers.
...
- [`docs/dac_adapter_plan.md`](docs/dac_adapter_plan.md) - the adapter's design log and current
  state (closest companion to this README, though some loss details predate the cosine+magnitude switch).
...
- [`docs/ADVERSARIAL_DISENTANGLEMENT_APPROACH.md`](docs/ADVERSARIAL_DISENTANGLEMENT_APPROACH.md),
  ...
  approaches, kept for historical context.
...
- **FiLM** - Feature-wise Linear Modulation, a per-channel scale+shift predicted from conditioning.
